...
- Full readiness passed: schema v5 review, current image and contract hashes, all five adjacent transitions, derivatives, social card, 278 local URLs with zero failures, JSON-LD, asset usage, ledger, and clean diff checks.
- Rendered desktop 1440x1000 and mobile 390x844 homepage, library, and tutorial QA passed on the documented localhost fallback because Cove's certificate is expired. No overflow, invalid JSON-LD, public `lab.html` links, or broken images; all 135 library images and all 11 tutorial images loaded after scrolling. Layout rated 9.0/10 desktop and 8.8/10 mobile.
- One current lesson per site remains active; no backfills. Generated older-page changes refresh day labels, latest-lesson cards, and previous/next navigation.
...
- Recorded the rejected `grapes-on-a-vine` plan and nine-grape contact sheet as
  private review material only. It is not a public lesson and needs a new,
  cumulative sheet before any future owner-authorized repair. The current-only
...
- 2026-08-06 daily automation added one current lesson only:
  ...
  contours, three mirrored stitch bands, leather texture, restrained color,
  and final reinforcement. The finished sketch rates 9.5/10 for immediate
  silhouette readability, tactile graphite, appealing western detail, strong
  thumbnail composition, and a cumulative 25-minute progression.
- Readiness passed the ledger, schema-v2 process plan, frame deltas,
  derivatives, social card, JSON-LD, internal links, unused assets, lab
  privacy, and diff checks. Cove QA at 1280x720 and 390x844 covered the
  homepage, 104-card library, and tutorial with no overflow, broken images,
  invalid JSON-LD, or public lab links. The one-current-lesson-per-site cadence
...
- 2026-07-26 daily automation added current `woolly-sheep-with-bell`
  ...
  Day 001). Both use reviewed six-panel handmade raster pencil contact sheets,
  cropped JPG masters, WebP derivatives, social cards, and explicit process
  plans. The sheep keeps one calm right-facing pose, four legs, collar, bell,
  ears, tail, and grouped wool rhythms stable through the finish; the
  microscope keeps one optical axis, two objectives, stage, arm, focus knob,
  mirror, hinge, and horseshoe base coherent through every frame. Finished
  image ratings: sheep 9.3/10 for immediate readability, tactile pencil
  texture, character, and attainable construction; microscope 9.6/10 for
  unusually stable perspective and hardware, rich graphite/brass texture, and
  an excellent cumulative sequence.
...
- 2026-07-17 mirrored Doodlea.day's correction-preflight fix:
  ...
  exact allow flag, and held daily lock all match. This keeps archive and
  same-day corrections inside the normal ledger and lock gates.
...
- July 10 Sketcha QA used the cross-site daily lock, duplicate-slot guard,
  ...
  browser QA passed at `https://sketcha.localhost/`; screenshots are under
  `/tmp/daily-sketch-doodle-qa-2026-07-10/`.
- Finished image ratings: classic pickup truck 8.8/10 for a clear side-view
  silhouette, stable wheel/cab/bed progression, and restrained blue-gray sketch
  tone; garden hand trowel 8.7/10 for readable blade/handle/ferrule structure,
  aligned process frames, and useful graphite/colored-pencil finish. The
...
- Finished image ratings: cozy reading lamp 8.7/10 for readable shade, base,
...
- 2026-07-03 owner-requested image promotion: restored two previously unused
  ...
  Day 001 and `mason-jar-sparklers` as May 30, 2026 / Day 002. Existing day
  labels shifted forward by two, so July 3 is now Day 036.
- The source sheets are preserved under `drafts/{slug}/`; cropped public
  assets are `assets/{slug}-step-1.jpg` through `step-5.jpg` and
  `assets/{slug}-finished-v1.jpg`. Finished image ratings: garden gnome
  8.5/10; mason jar with sparklers 8.6/10.
